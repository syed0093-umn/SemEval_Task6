...
DeBERTa-v3-base + Boolean Features + Focal Loss model.
"""
...
# Prepare text (same format as training)
...
# Boolean features are empty in eval set — default to 0.0
...
print("Loading model checkpoint: deberta_focal_features_best.pt")
model = DeBERTaWithFeatures(model_name=model_name, num_labels=3, bool_feature_dim=2)
model.load_state_dict(torch.load('deberta_focal_features_best.pt', map_location=device))
...
output_file = 'prediction_eval_focal_features'
with open(output_file, 'w') as f:
 for label in pred_labels:
 f.write(label + '\n')
print(f"Saved: {output_file}")

# --- Create submission zip (prediction file at root, no folders) ---
submission_zip = 'submission_eval_focal_features.zip'
with zipfile.ZipFile(submission_zip, 'w', zipfile.ZIP_DEFLATED) as zf:
 zf.write(output_file, arcname='prediction')
print(f"Created: {submission_zip}")
print("Done. Upload submission_eval_focal_features.zip to Codabench.")
