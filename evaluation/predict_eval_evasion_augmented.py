"""
Generate Subtask 2 (evasion) predictions on the evaluation dataset
using DeBERTa-v3-base trained on augmented evasion data.
"""
...
# --- Model definition (must match train_deberta_evasion_augmented.py) ---
...
# --- 9 evasion classes (sorted, matching training) ---
label_list = [
 'Claims ignorance', 'Clarification', 'Declining to answer',
 'Deflection', 'Dodging', 'Explicit', 'General', 'Implicit',
 'Partial/half-answer'
]
...
# --- Load model (DeBERTa-base, not large) ---
model_name = 'microsoft/deberta-v3-base'
print(f"Loading tokenizer: {model_name}")
tokenizer = AutoTokenizer.from_pretrained(model_name)

checkpoint = 'deberta_evasion_augmented_best.pt'
print(f"Loading model checkpoint: {checkpoint}")
model = DeBERTaWithFeatures(model_name=model_name, num_labels=9, bool_feature_dim=2)
...
pred_labels = [submission_label_map[id2label[p]] for p in all_preds]
...
output_file = 'prediction_eval_evasion_augmented'
...
submission_zip = 'submission_eval_evasion_augmented.zip'
with zipfile.ZipFile(submission_zip, 'w', zipfile.ZIP_DEFLATED) as zf:
 zf.write(output_file, arcname='prediction')
print(f"Created: {submission_zip}")
print("Done. Upload submission_eval_evasion_augmented.zip to Codabench.")
