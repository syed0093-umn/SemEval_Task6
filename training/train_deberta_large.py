...
print("="*80)
print("DeBERTa-v3-LARGE with Boolean Features + Focal Loss")
print("="*80)

# Parse command-line arguments
parser = argparse.ArgumentParser(description='DeBERTa-LARGE with Boolean Features + Focal Loss')
parser.add_argument('--learning_rate', type=float, default=2e-5, help='Learning rate (default: 2e-5, lower for large model)')
parser.add_argument('--batch_size', type=int, default=4, help='Batch size (default: 4, smaller for memory)')
parser.add_argument('--gradient_accumulation_steps', type=int, default=8, help='Gradient accumulation (default: 8, maintain effective batch=32)')
...
for epoch in range(NUM_EPOCHS):
 ...
 # Save best model
 torch.save(model.state_dict(), 'deberta_large_best.pt')
...
model.load_state_dict(torch.load('deberta_large_best.pt'))
...
import zipfile
submission_file = 'submission_large.zip'
...
print("\n" + "="*80)
print("DeBERTa-LARGE TRAINING COMPLETE")
print("="*80)
