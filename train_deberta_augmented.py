"""
DeBERTa-v3-base with Augmented Data Training

Based on train_deberta_focal_features.py with modifications for:
...
Usage:
 python train_deberta_augmented.py --data_dir ./QEvasion_augmented --synthetic_weight 0.5
"""
...
print("="*80)
print("DeBERTa-v3-base with AUGMENTED DATA + Focal Loss")
print("="*80)

# Parse command-line arguments
parser = argparse.ArgumentParser(description='DeBERTa with Augmented Data + Focal Loss')
...
parser.add_argument('--output_prefix', type=str, default='deberta_augmented',
 help='Prefix for output files (default: deberta_augmented)')
...
class FocalLossWithSampleWeights(nn.Module):
 ...
 sample_weights: [batch_size] - per-sample weights (1.0 for original, <1.0 for synthetic)
 """
...
class ClarityDatasetWithWeights(Dataset):
 ...
 # Sample weight: 1.0 for original, synthetic_weight for synthetic
...
def eval_epoch(model, dataloader, device, focal_loss):
 ...
 # No sample weights during evaluation (evaluate on original test set)
...
test_dataset = ClarityDatasetWithWeights(
 ...
 tokenizer=tokenizer,
 max_length=MAX_LENGTH,
 synthetic_weight=1.0 # Not used for test
)
...
print("\n" + "="*80)
print("AUGMENTED DATA TRAINING COMPLETE")
print("="*80)
